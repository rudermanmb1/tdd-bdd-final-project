...
class TestProductRoutes(TestCase):
    ...
    def test_query_by_category(self):
        ...
        category_set = [product for product in products if product.category == test_category]
        category_count = len(category_set)
        logging.debug("Found Products [%d] %s", category_count, category_set)
    ...
    def test_fail_to_get_product(self):
        """It should Get a single Product"""
        # get the id of a product
        test_product = self._create_products(1)[0]
    ...
    def test_fail_to_update_product(self):
        """It should Update an existing Product"""
        # create a product to update
        test_product = ProductFactory()
        response = self.client.post(BASE_URL, json=test_product.serialize())
        ...
        new_product = response.get_json()
        new_product['id'] = new_product['id']+1
        new_product["description"] = "unknown"
        response = self.client.put(f"{BASE_URL}/{new_product['id']}", json=new_product)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
